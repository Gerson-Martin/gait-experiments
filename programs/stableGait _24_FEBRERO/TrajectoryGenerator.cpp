...
void TrajectoryGenerator::generate(KDL::Trajectory_Composite & comTraj, KDL::Trajectory_Composite & leftTraj, KDL::Trajectory_Composite & rightTraj)
{
    ...
    KDL::Path* pathOneStep=getPathLegSwing(footSpec.lift,liftLeg,v4,H,200);
    ...
    singleFootTrajectory(comTraj,leftTraj,rightTraj,leftLeg,rightLeg,nStep);
...
//    liftLeg=H*a;
//    liftLeg.M=leftLeg.M;
//    pathFootSingle=new KDL::Path_Line(leftLeg, liftLeg, orient.Clone(), eqradius);
...
//    //Llevamos el pie a la posicion final manteniendo la posicion del torso//
//    liftLeg=leftTraj.Pos(leftTraj.Duration());
//    v4=H*steps[nStep+2];
//    v4.M=steps[nStep+2].M;
//    pathEndStep=/*new KDL::Path_Line(liftLeg, v4, orient.Clone(), eqradius);//*/
//                getPathLegSwing(footSpec.lift,liftLeg,v4,H,200);

//    KDL::VelocityProfile_TrapHalf *profVelEndStep=new KDL::VelocityProfile_TrapHalf(pathEndStep->PathLength()/DURATION_ONE_STEP*2,0.1,false);
...
//    profFootSingleCOM= new KDL::VelocityProfile_TrapHalf(pathFootSingleCOM->PathLength()/DURATION_ONE_STEP*4,0.2,true);
//    pathEndStep=new KDL::Path_Line(v4,steps[nStep+2], orient.Clone(), eqradius);
//    profVelEndStep=new KDL::VelocityProfile_TrapHalf(pathEndStep->PathLength()/DURATION_TURN_ANGLE_COM*2,0.1,false);
//    comTraj.Add(new KDL::Trajectory_Segment(pathFootSingleCOM, profFootSingleCOM, DURATION_TURN_ANGLE_COM));
//    leftTraj.Add(new KDL::Trajectory_Segment(pathEndStep, profVelEndStep->Clone(), DURATION_TURN_ANGLE_COM));
//    rightTraj.Add(new KDL::Trajectory_Stationary(DURATION_TURN_ANGLE_COM, rightLeg));
//    nStep++;
//    leftLeg=steps[nStep+1];
...
void TrajectoryGenerator::singleFootTrajectory(KDL::Trajectory_Composite &comTraj, KDL::Trajectory_Composite &LegInFlightTraj, KDL::Trajectory_Composite &staticLegTraj, KDL::Frame &LegInFlight, KDL::Frame &staticLeg, double &nStep)
{
    //Levantamos el pie y giramos el torso a la vez//
    KDL::Frame PointEnterFoot(comTraj.Pos(comTraj.Duration()));
    KDL::Path* pathFootSingleCOM=getTrayectoryWithCOMangle(PointEnterFoot,0.0, staticLeg,DEFAULT_TURN_ANGLE_COM);
    KDL::VelocityProfile* profFootSingleCOM= new KDL::VelocityProfile_TrapHalf((pathFootSingleCOM->PathLength()/DURATION_TURN_ANGLE_COM)*2,0.2,false);
    KDL::Frame H=pathFootSingleCOM->Pos(pathFootSingleCOM->PathLength())*PointEnterFoot.Inverse();
    ...
    //Desgiramos el torso y bajamos una pequeña distancia el pie al suelo
    pathFootSingleCOM=getTrayectoryWithCOMangle(beforeTurnCOM,0.0, staticLeg,-DEFAULT_TURN_ANGLE_COM);
    profFootSingleCOM= new KDL::VelocityProfile_TrapHalf(pathFootSingleCOM->PathLength()/DURATION_ONE_STEP*4,0.2,true);
    pathEndStep=new KDL::Path_Line(v4,steps[nStep+2], orient.Clone(), eqradius);
    profVelEndStep=new KDL::VelocityProfile_TrapHalf(pathEndStep->PathLength()/DURATION_TURN_ANGLE_COM*2,0.1,false);
    comTraj.Add(new KDL::Trajectory_Segment(pathFootSingleCOM, profFootSingleCOM, DURATION_TURN_ANGLE_COM));
