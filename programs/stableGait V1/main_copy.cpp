...
//    limitChecker.setReference(gaitSpec);
    gaitSpec.squat = 0.80;//iterateSquat();
    ...
    //data1 <<x0[0]<<DELIMITADOR<<x0[1]<< DELIMITADOR<<x0[2]<<DELIMITADOR<<x0[3]<<"\n";
    std::vector<KDL::Frame> trajCOM;

    double tSingleSupport=2.64;
    //int limit=tSingleSupport/period;

    KDL::RotationalInterpolation_SingleAxis orient;
    double eqradius=1.0;
    KDL::Path_Composite * pathCom = new KDL::Path_Composite();
    //KDL::Path_RoundedComposite * pathCom = new KDL::Path_RoundedComposite(0.001, eqradius, orient.Clone());
    KDL::Rotation rot;
//    for (double dt = 0; dt < tSingleSupport; dt+=period)
//    {
//        LIMP.output(u0);
//        std::vector<std::vector<double>> aux=LIMP.getState();
//        std::vector<double>x {aux[0][0],aux[1][0],aux[2][0],aux[3][0]};
//        x0=x;
//        trajCOM.emplace_back(rot,KDL::Vector(x[0],x[2],gaitSpec.squat));
//        pathCom->Add(new KDL::Path_Line(trajCOM[trajCOM.size()-2], trajCOM[trajCOM.size()-1], orient.Clone(), eqradius));
//        //data1 <<x[0]<<DELIMITADOR<<x[1]<< DELIMITADOR<<x[2]<<DELIMITADOR<<x[3]<<"\n";
//    }
...
//        std::vector<double>x {aux[0][0],aux[1][0],aux[2][0],aux[3][0]};
//        x0=x;
//        trajCOM.emplace_back(rot,KDL::Vector(x[2]+0.3,x[0]-0.12,gaitSpec.squat));
//        //std::cout<<"--------------"<<trajCOM[trajCOM.size()-1]<<std::endl;
//        //pathCom->Add(trajCOM[trajCOM.size()-1]);
//        pathCom->Add(new KDL::Path_Line(trajCOM[trajCOM.size()-2], trajCOM[trajCOM.size()-1], orient.Clone(), eqradius));
//        //data1 <<x[0]<<DELIMITADOR<<x[1]<< DELIMITADOR<<x[2]<<DELIMITADOR<<x[3]<<"\n";
//    }
//    x0[0]*=-1;
//    x0[2]*=-1;
//    //data1 <<x0[0]<<DELIMITADOR<<x0[1]<< DELIMITADOR<<x0[2]<<DELIMITADOR<<x0[3]<<"\n";
//    LIMP.InitialCondition(x0);
        //example(0.79,9.807);
        //findInitialConditions(0.2,0.01,0.1,0.79,9.807,0.02);
//    for (double dt = 0; dt < tSingleSupport; dt+=period)
//    {
//        LIMP.output(u0);
//        std::vector<std::vector<double>> aux=LIMP.getState();
//        std::vector<double>x {aux[0][0],aux[1][0],aux[2][0],aux[3][0]};
//        //data1 <<x[0]-2*x0[0]<<DELIMITADOR<<x[1]<< DELIMITADOR<<x[2]-2*x0[2]<<DELIMITADOR<<x[3]<<"\n";
//    }

    double vel=0.1;
    KDL::VelocityProfile_Rectangular profRect(vel);
//    KDL::Frame F_base_start;
//    KDL::Vector V_base_center(0.5,-1,0);
//    KDL::Vector V_base_p(1,-1,0);
//    double alpha=1;
//    KDL::Rotation R_base_end;
//    KDL::Path_Circle* a=new KDL::Path_Circle(F_base_start,V_base_center,
//                       V_base_p,
//                       R_base_end,
//                       alpha,
//                       orient.Clone(),
//                       eqradius);
//    double duration=a->PathLength()/2.5;
//    a->Write(std::cout);
//    KDL::Trajectory_Segment trayectoria(a, profRect.Clone(), duration);
//    double time;
//    while(time<duration){
//        auto t=rl::KdlVectorConverter::frameToVector(trayectoria.Pos(time));
//        data1 <<t[0]<<DELIMITADOR<<t[1]<< DELIMITADOR<<t[2]<<"\n";
//        time+=0.001;
//    }

//    double duration=pathCom->PathLength()/vel;
...
//            }
        ...
        catch (const KDL::Error_MotionPlanning & e)
        {
            ...
            break;//continue;
                ...
                std::cout<<rightTraj.Duration()<<" "<<leftTraj.Duration()<<" "<<" "<<comTraj.Duration()<<std::endl;
                double duration=rightTraj.Duration();
                double time=0.0;
                while(time<duration){
                    auto t=rl::KdlVectorConverter::frameToVector(rightTraj.Pos(time));
                    data1 <<t[0]<<DELIMITADOR<<t[1]<< DELIMITADOR<<t[2]<<"\n";
                    time+=0.001;
                }
...
//        }
        ...
        {
            yDebug() << "-2222222222222---------------------------------";
            yWarning() << "IK failed";
            break;//continue;
        }
        else
        {
            yDebug() << "-111111111111111111111---------------------------------";
            hasSolution = true;
            break;
        }
    }
