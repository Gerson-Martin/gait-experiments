          ...
          pathCom1->Add(new KDL::Path_Line(COM_P1,COM_P2, orient.Clone(), eqradius));
           ...
           if(phaseWalking[i]!=phaseWalking[i+1] || i==(com.size()-5)){
               ...
               if(phaseWalking[i]==-1){
                   ...
                   KDL::Trajectory_Segment *COM_DOUBLE=new KDL::Trajectory_Segment(pathCom1,profRectDS.Clone(),DEFAULT_DURATION_DS*m);
                   for (double i=0.0;i<=COM_DOUBLE->Duration();i+=Ts){
                   double zmp_X=COM_DOUBLE->Pos(i).p.x()+COM_DOUBLE->Acc(i).vel[0]*COM_DOUBLE->Pos(i).p.z()/DEFAULT_ACELERATION_GRAVITY;
                   double zmp_Y=COM_DOUBLE->Pos(i).p.y()+COM_DOUBLE->Acc(i).vel[1]*COM_DOUBLE->Pos(i).p.z()/DEFAULT_ACELERATION_GRAVITY;
                   data1 <<zmp_X<< DELIMITADOR<<zmp_Y<<"\n";
                   }
                   rightTraj.Add(new KDL::Trajectory_Stationary(DEFAULT_DURATION_DS*m, rightLeg));
                   leftTraj.Add(new KDL::Trajectory_Stationary(DEFAULT_DURATION_DS*m, leftLeg));
...
//    liftLeg.p[0]+=distance;
    ...
    for (double i=0.0;i<=COM_single->Duration();i+=Ts){
    double zmp_X=COM_single->Pos(i).p.x()+COM_single->Acc(i).vel[0]*COM_single->Pos(i).p.z()/DEFAULT_ACELERATION_GRAVITY;
    double zmp_Y=COM_single->Pos(i).p.y()+COM_single->Acc(i).vel[1]*COM_single->Pos(i).p.z()/DEFAULT_ACELERATION_GRAVITY;
    data1 <<zmp_X<< DELIMITADOR<<zmp_Y<<"\n";
    }
