...
//    double marginSep = footSpec.margin + gaitSpec.sep / 2.0;
    double stableSep = gaitSpec.sep;// footSpec.stable + gaitSpec.sep / 2.0;
    double stepSep = gaitSpec.sep;//(gaitSpec.sep + footSpec.width) / 2.0;
//    std::cout<<" footSpec.stable"<<footSpec.stable<<"  gaitSpec.squat "<< gaitSpec.squat <<std::endl;
    double initialSquat = -initialPose.p.z();
    double hopSquat =gaitSpec.squat;//; initialSquat - gaitSpec.squat;// + gaitSpec.hop;
//    double maxSquat = initialSquat - gaitSpec.squat;
//     yInfo() <<"; initialsquat:"<<initialSquat<<"; hop:"<< hopSquat<<"; max Squat:"<<maxSquat;
    double marginFwd = footSpec.width / 2.0 - footSpec.margin;
//    double stepFwd = footSpec.length - footSpec.margin - footSpec.width / 2.0;
//     yInfo() <<"; width:"<<footSpec.width<<"; footSpec.margin:"<< footSpec.length<<"; marginFwd:"<<marginFwd;
