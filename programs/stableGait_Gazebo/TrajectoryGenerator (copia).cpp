...
constexpr auto DURATION_ONE_STEP = 30.0; // [s]
...
//   //1 single support left
   ...
   {
       yDebug()<<"phaseTime--------------------------------: "<<phaseTime;
       KDL::Vector b(x[i+1][0],x[i+1][2],x[i+1][4]),a(x[i][0],x[i][2],x[i][4]);
       KDL::Frame c(a),d(b);
       pathCom1->Add(new KDL::Path_Line(c,d, orient.Clone(), eqradius));
       if(x[i][x[0].size()-1]!=x[i+1][x[0].size()-1]){
           phaseTime=i*Ts-before_time;
           before_time=i*Ts;
           KDL::VelocityProfile_Rectangular profRect(lenghtStep/phaseTime);
...
//       data1 <<x[i][0]<< DELIMITADOR<<x[i][2]<< DELIMITADOR<<x[i][x[0].size()-1]<<"\n";
...
//   rightTraj.Add(new KDL::Trajectory_Stationary(comTraj.Duration(), rightLeg));
//   leftTraj.Add(new KDL::Trajectory_Stationary(comTraj.Duration(), leftLeg));
//   KDL::Vector a1(x[x.size()-1][0],x[x.size()-1][2],x[x.size()-1][4]);
//   KDL::Frame b1(a1);
//   auto dist2=b1.p-com[com.size() - 3].p;
//   auto d1=dist2.Norm();
//   double  tiempo1=d1/profRect0.maxvel;
//   KDL::Path_Line* w2 =new KDL::Path_Line(com[com.size() - 3], b1, orient.Clone(), eqradius);
//   comTraj.Add(new KDL::Trajectory_Segment(w2,profRect0.Clone(),tiempo1));
...
std::vector<double> TrajectoryGenerator::findInitialConditions(double stepLength,double dy_mid,double x0,double zModel,double g, double Ts){
    ...
    std::cout<<" x0:"<<x0<<" y0:"<<y0<<" dx0:"<<dx0<<" dy0:"<<dy0<<std::endl;*/
