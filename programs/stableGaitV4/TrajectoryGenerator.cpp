      ...
      parcialCOMtrayectory->Add(new KDL::Trajectory_Segment(pathCom1, profRect2.Clone(), Ts));
       ...
       if(x[i][x[0].size()-1]!=x[i+1][x[0].size()-1] || i==(x.size()-2)){
           ...
           if(x[i][x[0].size()-1]==-1){
               ...
               KDL::VelocityProfile_Rectangular profRect(PathLegSwing->PathLength()/(phaseTime*2/3));
               leftTraj.Add(new KDL::Trajectory_Segment(PathLegSwing, profRect.Clone(), phaseTime*2/3));
...
//               comTraj.Add(parcialCOMtrayectory);
               ...
               KDL::VelocityProfile_Rectangular profRect(PathLegSwing->PathLength()/(phaseTime*2/3));
               rightTraj.Add(new KDL::Trajectory_Segment(PathLegSwing, profRect.Clone(), phaseTime*2/3));
               rightLeg=steps[nStep+1];
...
//            rot.DoRotX(-diff_angle);
            rot2=rot.RotX((angle-diff_angle*(i-n/2))*2);
