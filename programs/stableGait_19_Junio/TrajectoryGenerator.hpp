...
public:
    ...
    void configure(const std::vector<KDL::Frame> & steps, const std::vector<std::tuple<KDL::Frame,double>> &com);
   void generate(KDL::Trajectory_Composite & comTraj, KDL::Trajectory_Composite & leftTraj, KDL::Trajectory_Composite & rightTraj);
    std::ofstream data1;
    KDL::Path_Composite * pathZMP= new KDL::Path_Composite();
private:
    ...
    void velocityProfile(double vel_init, double vel_end, KDL::Path&, double duration);
    ...
    std::vector<double> phaseWalking;
    std::vector<std::tuple<KDL::Frame,double>> COM;
