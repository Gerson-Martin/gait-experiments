...
//    KDL::VelocityProfile_Rectangular* profEnterFoot = new KDL::VelocityProfile_Rectangular(pathEnterFoot->PathLength()/(DEFAULT_DURATION_DS/2));
    KDL::VelocityProfile_Trap* profEnterFoot = new KDL::VelocityProfile_Trap(pathEnterFoot->PathLength()/(DEFAULT_DURATION_DS)*2,0.1);
    comTraj.Add(new KDL::Trajectory_Segment(pathEnterFoot, profEnterFoot, DEFAULT_DURATION_DS));
...
//    data1 <<prueba.Pos(i).p[2]/*prueba.Vel(i).vel[2]*/ /*a.Vel(i)*/<<DELIMITADOR<<Ts*i<<DELIMITADOR<<prueba.Vel(i).vel[2]<<"\n";
//    }
    ...
    data1 <<zmp_X<< DELIMITADOR<<zmp_Y<<DELIMITADOR<<i<<DELIMITADOR<<comTraj.Acc(i).vel[0]<<DELIMITADOR<<comTraj.Acc(i).vel[1]<<DELIMITADOR<<comTraj.Acc(i).vel[2]<<"\n";
    }
//    {
//        auto && log = yInfo();
...
KDL::Path_Composite* TrajectoryGenerator::getTrayectoryWithCOMangle(KDL::Frame &point,double distance, KDL::Frame Leg, double angle)
{
    ...
    for(double i=0;i<=n;i+=1){
        z=/*start.p.z();//*/L*sin(initial_angle+diff_angle*i)+Leg.p.z();
        y=L*cos(initial_angle+diff_angle*i)+Leg.p.y();
...
//    liftLeg.p[0]+=distance;
    ...
    KDL::VelocityProfile_Trap* profFootSingle=new KDL::VelocityProfile_Trap(pathFootSingle->PathLength()/DURATION_TURN_ANGLE_COM*2,0.1);///2,0.1/2)//new KDL::VelocityProfile_Rectangular((pathFootSingle->PathLength()/DURATION_TURN_ANGLE_COM));
    KDL::Trajectory_Segment *COM_single= new KDL::Trajectory_Segment(pathFootSingleCOM, profFootSingleCOM, DURATION_TURN_ANGLE_COM);

    comTraj.Add(COM_single);
...
void TrajectoryGenerator::velocityProfile(double vel_init, double vel_end, KDL::Path &path,double duration)
{
//    if(vel_end<vel_init){
//        for(double t=0.0;t<duration;t+=Ts){
//        generateCurveLeg(vel_init,duration,vel_end,time);
